defmodule Game.CommandTest do
  ...
  describe "parsing commands" do
    ...
    test "whisper", %{user: user} do
      assert %Command{module: Command.Whisper, args: {:whisper, "player hi"}} = Command.parse("whisper player hi", user)
    end
    ...
    test "listen", %{user: user} do
      assert %Command{module: Command.Listen, args: {}} = Command.parse("listen", user)
    end

    test "debug", %{user: user} do
      assert %Command{module: Command.Debug, args: {:info}} = Command.parse("debug info", user)
    end

    test "scan", %{user: user} do
      assert %Command{module: Command.Scan, args: {}} = Command.parse("scan", user)
    end
  end
