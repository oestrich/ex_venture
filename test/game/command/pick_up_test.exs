defmodule Game.Command.PickUpTest do
  ...
  test "pick up an item from a room", %{session: session, socket: socket} do
    {:update, state} = Game.Command.PickUp.run(["sword"], "pick up sword", session, %{socket: socket, save: %Save{room_id: 1, item_ids: []}})
  ...
  test "item does not exist in the room", %{session: session, socket: socket} do
    :ok = Game.Command.PickUp.run(["shield"], "pick up shield", session, %{socket: socket, save: %Save{room_id: 1, item_ids: []}})
  ...
  test "item has already been removed", %{socket: socket} do
    ...
    room = %Data.Room{id: 1}
    :ok = Game.Command.PickUp.pick_up(item, room, %{socket: socket, save: %Save{room_id: 1, item_ids: []}})
