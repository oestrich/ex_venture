defmodule Web.Controller.RoomExitControllerTest do
  ...
  test "create a new exit", %{conn: conn, room1: room1, room2: room2} do
    conn = post conn, room_exit_path(conn, :create, room1.id, direction: "south"), exit: %{direction: "north", start_id: room1.id, finish_id: room2.id}
    assert html_response(conn, 302)
  end

  test "delete an exit", %{conn: conn, room1: room1, room2: room2} do
    room_exit = create_exit(%{direction: "north", start_id: room1.id, finish_id: room2.id})
